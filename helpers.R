...
group_tiebreak <- function(standings, df_results) {
  ...
  for(g in LETTERS[1:6]) {
    ...
    } else if(group_standings$points[1] == group_standings$points[2]) {
      tiebreak_order <- 
        group_results %>% 
    ...
    } else if(group_standings$points[2] == group_standings$points[3]) {
      tiebreak_order <- 
        group_results %>% 
